...
typedef struct {
    // A generic pointer that can hold a C4_MenuScreen, C4_Gamescreen, or anything else
    void* data;
    void (*Draw)(void* screenData);
    C4_ScreenChangeRequest (*HandleMouseEvents)(void* screenData, SDL_Event* event);
    C4_ScreenChangeRequest (*HandleKeyboardInput)(void* screenData, SDL_Scancode scancode);
