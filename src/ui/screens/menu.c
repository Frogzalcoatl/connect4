...
C4_Screen_Menu* C4_Screen_Menu_Create(SDL_Renderer* renderer) {
    ...
    if (!C4_UI_Text_InitProperties(&screen->title, screen->renderer, "Connect4", C4_FontType_Bold, C4_SCREEN_MENU_TITLE_PT_SIZE, 0.f, 50.f, 0)) {
    ...
    if (
        !C4_UI_Popup_InitProperties(
            ...
            3, C4_UI_ButtonGroup_Direction_Horizontal, 2, 100.f, "Do you want to exit Connect4?", C4_FONT_DEFAULT_PT_SIZE, C4_FONT_DEFAULT_PT_SIZE
        )
    ) {
        C4_Screen_Menu_Destroy(screen);
        return NULL;
    ...
    for (size_t i = 0; i < C4_SCREEN_MENU_POPUPBUTTONCOUNT; i++) {
        C4_UI_ButtonGroup_SetButtonIndex(
            &screen->exitGamePopup.buttonGroup, i, screen->renderer, popupButtonText[i], C4_DEFAULT_BUTTON_FONT,
            C4_FONT_DEFAULT_PT_SIZE, 3, C4_UI_SymbolType_None, 0.f, 0.f, 0
        );
    }
