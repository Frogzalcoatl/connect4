...
static void C4_VFS_InitThroughFolder(void) {
  ...
  SDL_snprintf(assetsFolderPath, sizeof(assetsFolderPath), "%s%s", vfs.basePath,
               "/assets");
...
static void C4_VFS_InitThroughDatFile(void) {
  ...
  if (!vfs.entries) {
    C4_FatalError(C4_ErrorCode_OutOfMemory,
                  "Failed to allocate VFS Table of Contents");
  }
...
static void *C4_VFS_ReadFileFromFolder(const char *filename, size_t *outSize) {
  ...
  SDL_snprintf(fullFilePath, sizeof(fullFilePath), "%s%s%s", vfs.basePath, "/",
               filename);

  // Using read binary "rb". "r" can corrupt binary data by translating line
  // endings \n to \r\n
...
void *C4_VFS_ReadFile(const char *filename, size_t *outSize) {
  ...
  if (!vfs.packFile) {
    C4_Warn(SDL_LOG_CATEGORY_APPLICATION, "Unable to read file %s", filename);
    return NULL;
  }
  ...
  C4_PackEntry *entry = bsearch(&id, vfs.entries, vfs.fileCount,
                                sizeof(C4_PackEntry), C4_VFS_CompareEntries);

  if (!entry) {
    C4_Warn(SDL_LOG_CATEGORY_APPLICATION, "Unable to read file in %s:\n%s",
            MASTER_DAT_PATH, filename);
    return NULL;
  }
  ...
  SDL_SeekIO(vfs.packFile, entry->offset, );

  size_t bytesRead = SDL_ReadIO(vfs.packFile, buffer, entry->size);
  if (bytesRead != entry->size) {
    SDL_free(buffer);
    C4_FatalError(C4_ErrorCode_DatFileInvalid,
                  "Unable to read full data for asset: %s", filename);
  }
