...
static const char* MUSIC_ASSETS[C4_MusicTrack_Count] = {"assets/music/test.ogg"};
...
C4_SoundSystem* C4_SoundSystem_Create(void) {
    ...
    if (!soundSystem) {
        C4_Warn(SDL_LOG_CATEGORY_AUDIO, "Unable to init sound system. Pointer is NULL.");
        return NULL;
    }

    soundSystem->mixerDevice = MIX_CreateMixerDevice(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, NULL);
    if (!soundSystem->mixerDevice) {
        C4_Warn(SDL_LOG_CATEGORY_AUDIO, "Mixer creation failed: %s", SDL_GetError());
        return NULL;
    }
    ...
    for (size_t i = 0; i < C4_SoundEffect_Count; i++) {
        ...
        if (!rawData) {
            C4_Warn(SDL_LOG_CATEGORY_AUDIO, "Unable to read sound: %s", SOUND_ASSETS[i]);
            continue;
        }
        ...
        if (!io) {
            ...
            C4_Warn(
                SDL_LOG_CATEGORY_AUDIO, "Unable to create SDL io for sound: %s", SOUND_ASSETS[i]
            );
            continue;
        }
...
void C4_SoundSystem_PlaySound(C4_SoundSystem* soundSystem, C4_SoundEffect soundID) {
    ...
    if (!soundSystem->loadedSounds[soundID]) {
        C4_Warn(
            SDL_LOG_CATEGORY_AUDIO, "Attempted to play unloaded sound: %s", SOUND_ASSETS[soundID]
        );
        return;
    }
...
void C4_SoundSystem_PlayMusic(C4_SoundSystem* soundSystem, C4_MusicTrack musicID) {
    ...
    if (!io) {
        C4_Warn(SDL_LOG_CATEGORY_AUDIO, "Failed to create IO for music: %s", MUSIC_ASSETS[musicID]);
        C4_VFS_FreeFile(rawData);
        return;
    }
    ...
    if (!soundSystem->currentMusic) {
        C4_Warn(SDL_LOG_CATEGORY_AUDIO, "Mixer failed to load audio IO: %s", SDL_GetError());
        C4_VFS_FreeFile(rawData);
        return;
    }
...
void C4_SoundSystem_SetVolume(C4_SoundSystem* soundSystem, C4_AudioTrack trackID, float volume) {
    if (!soundSystem || !soundSystem->mixerDevice) {
        C4_Warn(
            SDL_LOG_CATEGORY_AUDIO, "Unable to set volume. One or more required pointers are NULL"
        );
        return;
    }
...
static double C4_GetMusicPosition(C4_SoundSystem* soundSystem) {
    if (!soundSystem) {
        C4_Warn(
            SDL_LOG_CATEGORY_AUDIO, "Unable to get music position. Sound system pointer is NULL"
        );
        return 0.0;
    }
...
float C4_GetBPMBounce(C4_SoundSystem* soundSystem, float bpm) {
    if (!soundSystem) {
        C4_Warn(SDL_LOG_CATEGORY_AUDIO, "Unable to get bpm bounce. Sound system pointer is NULL");
        return 0.0f;
    }
    if (!soundSystem->isMusicPlaying) {
        return 0.0f;
